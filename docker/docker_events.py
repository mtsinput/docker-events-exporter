...
def print_timed(msg):
    to_print = "{} [{}]: {}".format(
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "docker_events", msg
    )
    print(to_print)
...
def delete_old_logs():
    ...
    if len(list_of_logs) > days_to_store():
        ...
        print_timed("Delete old logs file:" + " /log-events/" + oldest_file)
        os.remove(os.path.abspath(oldest_file))


def watch_events():
    client = docker.DockerClient(version="auto", base_url="unix://var/run/docker.sock")
    for event in client.events(decode=True):
        ...
        if log_to_disk().upper() == "TRUE" or log_to_disk().upper() == "YES":
            ...
            if log_date == "{}".format(datetime.now().strftime("%Y-%m-%d")):
                ...
                log_date = "{}".format(datetime.now().strftime("%Y-%m-%d"))
                print_timed(
                    "logs writing to " + "/log-events/docker_event_" + log_date + ".log"
                )
        ...
        if "com.docker.swarm.task.id" in attributes:
            ...
            print_timed(msg)
            full_pod = attributes["com.docker.swarm.task.name"]
            pod = re.findall(r"^(.*)\.[0-9a-z]*$", full_pod)[0]
            if pod in (None, ''):
                pod = full_pod
            #    if event['status'] == 'oom':
            ...
            EVENTS.labels(
                ...
                exitcode=exit_code,
                signal=signal,
                env=attributes["com.docker.stack.namespace"],
            ).inc()
        if "image" and "name" in attributes and not "com.docker.swarm.task.id" in attributes:
            msg = "{} on {} ({}) {}".format(
                event_status, attributes["name"], exit_code, signal
            )
            print_timed(msg)
            pod = attributes["name"]
            #    if event['status'] == 'oom':
